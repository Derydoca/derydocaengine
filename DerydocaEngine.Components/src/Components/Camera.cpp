...
namespace DerydocaEngine::Components
{
	...
	void Camera::deserialize(const YAML::Node& node)
	{
		...
		{
			m_RenderingMode = static_cast<Rendering::RenderingMode>(renderingModeNode.as<int>());
		}

		auto renderTextureNode = node["RenderTexture"];
		if (renderTextureNode)
		{
			m_RenderTextureSize.x = renderTextureNode["Width"].as<int>();
			m_RenderTextureSize.y = renderTextureNode["Height"].as<int>();
			m_RenderTexture = std::make_shared<Rendering::RenderTexture>(Rendering::RenderingMode::Forward);
			m_RenderTexture->initializeTexture(m_RenderTextureSize.x, m_RenderTextureSize.y);
			...
			auto postProcessShaderResource = getResource<Resources::ShaderResource>(renderTextureNode, "PostProcessShader");
			m_PostProcessShader.Set(postProcessShaderResource);
			if (m_PostProcessShader)
			{
				m_PostProcessMaterial = std::make_shared<Rendering::Material>();
				auto postPricessShader = std::static_pointer_cast<Rendering::Shader>(postProcessShaderResource->getResourceObjectPointer());
				m_PostProcessMaterial->setShader(postPricessShader);
			}
		}
		...
		if (skyboxNode)
		{
			auto skyboxMaterialResource = ObjectLibrary::getInstance().getResource<Resources::MaterialResource>(skyboxNode.as<boost::uuids::uuid>());
			m_SkyboxMaterial.Set(skyboxMaterialResource);
			auto skybox = getResource<Resources::MaterialResource>(node, "skybox");
			setSkybox(skybox);
		}
		...
		{
			auto deferredCompositorShaderResource = getResource<Resources::ShaderResource>(node, "deferredCompositorShader");
			m_DeferredRendererCompositor.Set(deferredCompositorShaderResource);
